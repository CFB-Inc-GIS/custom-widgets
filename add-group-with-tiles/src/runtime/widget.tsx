...
export default function Widget(this: any, props: AllWidgetProps<any>) {
  ...
  const createLayerFromUrl = ( url: string, type: SupportedLayerType ): SupportedLayer => {
  switch (type) {
    case 'MAP_SERVICE':
  ...
  React.useEffect(() => {
    ...
    const run = async (dataSourceId: string) => {
      ...
      await layer.load();
      const subLayers = layer.layers.toArray().map((sublayer: any) => {
